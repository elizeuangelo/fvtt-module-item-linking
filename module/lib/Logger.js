...
export default class Logger {
	static get DEBUG() {
		return (
			game.settings.get(MODULE_ID, 'debug') ||
			game.modules.get('_dev-mode')?.api?.getPackageDebugValue(MODULE_ID, 'boolean')
		);
	}
	...
	static debug(msg, ...args) {
		try {
			if (
				...
				console.log(`DEBUG | ${MODULE_ID} | ${msg}`, ...args);
			}
		} catch (e) {
	...
	static dialogWarning(message, icon = 'fas fa-exclamation-triangle') {
		return `<p class="${MODULE_ID}-dialog">
        <i style="font-size:3rem;" class="${icon}"></i><br><br>
        <strong style="font-size:1.2rem;">${MODULE_ID}</strong>
        <br><br>${message}
    </p>`;
	}
}
